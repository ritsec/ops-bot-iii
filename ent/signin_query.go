...
func (sq *SigninQuery) QueryUser() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(signin.Table, signin.FieldID, selector),
...
// First returns the first Signin entity from the query.
// Returns a *NotFoundError when no Signin was found.
func (sq *SigninQuery) First(ctx context.Context) (*Signin, error) {
	...
	if len(nodes) == 0 {
		return nil, &NotFoundError{signin.Label}
	}
	return nodes[0], nil
}
...
// FirstID returns the first Signin ID from the query.
// Returns a *NotFoundError when no Signin ID was found.
...
// Only returns a single Signin entity found by the query, ensuring it only returns one.
// Returns a *NotSingularError when more than one Signin entity is found.
// Returns a *NotFoundError when no Signin entities are found.
func (sq *SigninQuery) Only(ctx context.Context) (*Signin, error) {
	...
	case 0:
		return nil, &NotFoundError{signin.Label}
	default:
		return nil, &NotSingularError{signin.Label}
	}
}
...
// OnlyID is like Only, but returns the only Signin ID in the query.
// Returns a *NotSingularError when more than one Signin ID is found.
...
func (sq *SigninQuery) OnlyID(ctx context.Context) (id int, err error) {
	...
	case 0:
		err = &NotFoundError{signin.Label}
	default:
		err = &NotSingularError{signin.Label}
	}
	return
}
...
func (sq *SigninQuery) All(ctx context.Context) ([]*Signin, error) {
	...
	qr := querierAll[[]*Signin, *SigninQuery]()
	return withInterceptors[[]*Signin](ctx, sq, qr, sq.inters)
}
...
func (sq *SigninQuery) IDs(ctx context.Context) (ids []int, err error) {
	...
	if err = sq.Select(signin.FieldID).Scan(ctx, &ids); err != nil {
		return nil, err
	}
	return ids, nil
}
...
func (sq *SigninQuery) Count(ctx context.Context) (int, error) {
	...
	return withInterceptors[int](ctx, sq, querierCount[*SigninQuery](), sq.inters)
}
...
// Clone returns a duplicate of the SigninQuery builder, including all associated steps. It can be
...
func (sq *SigninQuery) Clone() *SigninQuery {
	...
	return &SigninQuery{
		config:     sq.config,
		ctx:        sq.ctx.Clone(),
		order:      append([]signin.OrderOption{}, sq.order...),
		inters:     append([]Interceptor{}, sq.inters...),
		predicates: append([]predicate.Signin{}, sq.predicates...),
...
func (sq *SigninQuery) GroupBy(field string, fields ...string) *SigninGroupBy {
	...
	grbuild := &SigninGroupBy{build: sq}
	grbuild.flds = &sq.ctx.Fields
	grbuild.label = signin.Label
...
func (sq *SigninQuery) Select(fields ...string) *SigninSelect {
	...
	sbuild := &SigninSelect{SigninQuery: sq}
	sbuild.label = signin.Label
...
func (sq *SigninQuery) prepareQuery(ctx context.Context) error {
...
func (sq *SigninQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Signin, error) {
	var (
		nodes       = []*Signin{}
		withFKs     = sq.withFKs
		_spec       = sq.querySpec()
	...
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, signin.ForeignKeys...)
	}
	_spec.ScanValues = func(columns []string) ([]any, error) {
		return (*Signin).scanValues(nil, columns)
	}
	_spec.Assign = func(columns []string, values []any) error {
		node := &Signin{config: sq.config}
	...
	if query := sq.withUser; query != nil {
		if err := sq.loadUser(ctx, query, nodes, nil,
			func(n *Signin, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SigninQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Signin, init func(*Signin), assign func(*Signin, *User)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Signin)
	for i := range nodes {
		if nodes[i].user_signins == nil {
			continue
		}
		fk := *nodes[i].user_signins
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_signins" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (sq *SigninQuery) sqlCount(ctx context.Context) (int, error) {
	_spec := sq.querySpec()
...
func (sq *SigninQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(signin.Table, signin.Columns, sqlgraph.NewFieldSpec(signin.FieldID, field.TypeInt))
	_spec.From = sq.sql
...
func (sq *SigninQuery) sqlQuery(ctx context.Context) *sql.Selector {
	builder := sql.Dialect(sq.driver.Dialect())
	t1 := builder.Table(signin.Table)
...
func (sgb *SigninGroupBy) Scan(ctx context.Context, v any) error {
	...
	return scanWithInterceptors[*SigninQuery, *SigninGroupBy](ctx, sgb.build, sgb, sgb.build.inters, v)
}

func (sgb *SigninGroupBy) sqlScan(ctx context.Context, root *SigninQuery, v any) error {
...
func (ss *SigninSelect) Scan(ctx context.Context, v any) error {
	...
	return scanWithInterceptors[*SigninQuery, *SigninSelect](ctx, ss.SigninQuery, ss, ss.inters, v)
}

func (ss *SigninSelect) sqlScan(ctx context.Context, root *SigninQuery, v any) error {
