...
func (uq *UserQuery) loadSignins(ctx context.Context, query *SigninQuery, nodes []*User, init func(*User), assign func(*User, *Signin)) error {
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_signins" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (uq *UserQuery) loadVotes(ctx context.Context, query *VoteQuery, nodes []*User, init func(*User), assign func(*User, *Vote)) error {
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_votes" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
