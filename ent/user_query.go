...
func (uq *UserQuery) QuerySignins() *SigninQuery {
	query := (&SigninClient{config: uq.config}).Query()
...
func (uq *UserQuery) QueryOpenstack() *OpenstackQuery {
	query := (&OpenstackClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withOpenstack: uq.withOpenstack.Clone(),
...
// WithSignins tells the query-builder to eager-load the nodes that are connected to
// the "signins" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithSignins(opts ...func(*SigninQuery)) *UserQuery {
	query := (&SigninClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withSignins = query
	return uq
}
...
// WithOpenstack tells the query-builder to eager-load the nodes that are connected to
// the "openstack" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithOpenstack(opts ...func(*OpenstackQuery)) *UserQuery {
	query := (&OpenstackClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withOpenstack = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withSignins; query != nil {
		if err := uq.loadSignins(ctx, query, nodes,
			func(n *User) { n.Edges.Signins = []*Signin{} },
			func(n *User, e *Signin) { n.Edges.Signins = append(n.Edges.Signins, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := uq.withOpenstack; query != nil {
		if err := uq.loadOpenstack(ctx, query, nodes, nil,
			func(n *User, e *Openstack) { n.Edges.Openstack = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadSignins(ctx context.Context, query *SigninQuery, nodes []*User, init func(*User), assign func(*User, *Signin)) error {
	...
	query.withFKs = true
	query.Where(predicate.Signin(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.SigninsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_signins
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_signins" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_signins" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (uq *UserQuery) loadShitposts(ctx context.Context, query *ShitpostQuery, nodes []*User, init func(*User), assign func(*User, *Shitpost)) error {
	...
	for _, n := range neighbors {
		fk := n.user_shitposts
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_shitposts" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_shitposts" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (uq *UserQuery) loadBirthday(ctx context.Context, query *BirthdayQuery, nodes []*User, init func(*User), assign func(*User, *Birthday)) error {
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_birthday" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (uq *UserQuery) loadOpenstack(ctx context.Context, query *OpenstackQuery, nodes []*User, init func(*User), assign func(*User, *Openstack)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.Openstack(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.OpenstackColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_openstack
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_openstack" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_openstack" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
